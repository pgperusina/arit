...
public class Linea extends Funcion {
    ...
    public Object ejecutar(Tabla tabla, Arbol arbol) {
        if (tabla.getVariableLocal(LINEA_PARAMETRO+5).getValor() instanceof Vector
                & (tabla.getVariable(LINEA_PARAMETRO+5).getTipo().getTipoDato().equals(Tipo.TipoDato.INTEGER)
                | tabla.getVariable(LINEA_PARAMETRO+5).getTipo().getTipoDato().equals(Tipo.TipoDato.NUMERIC))) {
            /**
             * Es un diagrama de dispersión
             */
            Simbolo simboloDatos = tabla.getVariableLocal(LINEA_PARAMETRO+1);
            Simbolo simboloLabelX = tabla.getVariableLocal(LINEA_PARAMETRO+2);
            Simbolo simboloLabelY = tabla.getVariableLocal(LINEA_PARAMETRO+3);
            Simbolo simboloTitulo = tabla.getVariableLocal(LINEA_PARAMETRO+4);
            Simbolo simboloLimites = tabla.getVariableLocal(LINEA_PARAMETRO+5);

            if (simboloDatos == null | simboloLabelX == null | simboloLabelY == null
                    | simboloLimites == null | simboloTitulo == null) {
            ...
            if (!( simboloLabelX.getTipo().getTipoEstructura().equals(Tipo.TipoEstructura.VECTOR)
                    ...
                    | simboloLimites.getTipo().getTipoEstructura().equals(Tipo.TipoEstructura.VECTOR))) {
            ...
            if (!(simboloLimites.getTipo().getTipoDato().equals(Tipo.TipoDato.INTEGER)
                    | simboloLimites.getTipo().getTipoDato().equals(Tipo.TipoDato.NUMERIC))) {
                return new Excepcion("Semántico", "El argumento YLim enviado a la funcion " +
                        this.nombre.toUpperCase() + " debe de ser de tipo Integer o Numeric.", simboloLimites.getFila(), simboloLimites.getColumna());
            }
            ...
            Vector limites = (Vector)simboloLimites.getValor();

            if (limites.size() != 2) {
                return new Excepcion("Semántico", "El argumento YLim enviado a la funcion " +
                        this.nombre.toUpperCase() + " debe de ser de tener 2 posiciones.", simboloLimites.getFila(), simboloLimites.getColumna());
            }
            ...
            yAxis.setLowerBound(Double.valueOf(limites.getFirst().toString()));
            yAxis.setUpperBound(Double.valueOf(limites.get(1).toString()));
            ...
            arbol.getListaGraficas().put("Dispersión " + lineChart.getTitle(), lineChart);
            ...
            lineChart.setTitle(String.valueOf(titulo.getFirst()));
            lineChart.setPrefSize(650, 500);
            arbol.getListaGraficas().put("Line Chart " + lineChart.getTitle(), lineChart);
            Simbolo s = new Simbolo(null, this.nombre, null);
            s.setFila(simboloDatos.getFila());
            s.setColumna(simboloDatos.getColumna());
            return s;
